...
resource "google_compute_address" "vito_balerica_vpn_static_ip" {
  ...
  region     = "southamerica-east1"
  provider   = google.vito
  depends_on = [google_compute_network.vito_balerica_inc_main]
}
...
resource "google_compute_vpn_tunnel" "vito_balerica_to_yahshua_tunnel" {
  name          = "vito-balerica-to-yahshua-tunnel"
  peer_ip       = google_compute_address.yahshua_vpn_static_ip.address
...
resource "google_compute_route" "vito_balerica_to_yahshua_route" {
  ...
  next_hop_vpn_tunnel = google_compute_vpn_tunnel.vito_balerica_to_yahshua_tunnel.id
}
